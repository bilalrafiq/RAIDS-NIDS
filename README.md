...
codebase for the frozen v0.18 through v0.23 experimental record. Later
...
| v0.23 | NF-UNSW-NB15-v3 Reconnaissance | Prespecified second-episode Gate 4 replication | Gates 1 through 3 passed; 90 Gate 4 runs, three confirmatory contrasts, and six secondary analyses completed |
...
│   └── v023_unsw_reconnaissance_gate4/ # compact, checksum-bound v0.23 evidence
├── reproducibility/
...
python scripts/verify_v023_publication_evidence.py \
  --root results/frozen/v023_unsw_reconnaissance_gate4 \
  --expected-archive-sha256 41594744d24581d74c31624be24dbfec6013ed2b5fc0aaba4780934d1f37200b \
  --expected-archive-size 117834865
...
- [v0.23 public evidence packaging](docs/V023_PUBLIC_EVIDENCE_PACKAGING.md)
...
## Frozen v0.23 result boundary

The outcome-free v0.23 prespecification was frozen before the NF-UNSW-NB15-v3
Reconnaissance run. All ten Gate 2/3 seeds passed the authoritative MAD guard,
and all 90 Gate 4 runs completed. The query-provenance audit passed all 90
count, uniqueness, hash, seed, and contract checks.

The three prespecified primary contrasts used normalized recovery area and
Holm correction:

| Contrast | Mean paired difference | 95% bootstrap interval | Holm-adjusted p |
|---|---:|---:|---:|
| Uncertainty-diversity minus random | -0.008057 | [-0.012944, -0.003266] | 0.017578 |
| Budget 200 minus budget 50 | 0.009213 | [0.004408, 0.013949] | 0.011719 |
| Uncertainty-diversity B200 minus static | 0.026167 | [0.018063, 0.033366] | 0.005859 |

The selector result reversed relative to the v0.22 Exploits episode, while the
larger-budget and uncertainty-diversity B200-versus-static contrasts remained
positive. The evidence therefore supports episode-conditional adaptation
claims rather than universal selector superiority. v0.23 uses a second family
episode from the shared NF-UNSW trace, so it is not an independent deployment
environment or untouched validation study.

The compact v0.23 directory retains 630 source files byte for byte, maps 100
long run directories to short identifiers, and inventories the 100 model
binaries and five logs kept only in the separate full archive.
...
directories do not include model binaries or logs; their hashes and archive
paths are recorded in each directory's `OMITTED_FILES.csv`.
