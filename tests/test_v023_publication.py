...
def build_source_evidence(tmp_path: Path) -> tuple[Path, Path, str, int]:
    root = tmp_path / "source" / "v023_unsw_reconnaissance_gate4"
    ...
    write_json(
        root / "audit" / "query_provenance_audit.json",
        {
            "status": "passed",
    ...
    write_json(
        root / "preflight.json",
        {
            ...
            "dry_run": False,
            "status": "passed",
            "git_head": "ecca58e9f3ebcc3a70ab8d9ca0d1fcbc34a9b645",
            "required_base_commit": "00b90bfb7a6f3aeb9eebb14af12fae228b529702",
            "planned_runs": {
                "adaptive_runs": 80,
    ...
    for seed in EXPECTED_SEEDS:
        directory = (
            ...
            / "runs"
            / (
                "E23_UNSW_Reconnaissance_core_profile_gate2_gate3__"
    ...
    for method in EXPECTED_GATE4_METHODS:
        for seed in EXPECTED_SEEDS:
            directory = (
                ...
                / "runs"
                / (
                    f"E23_UNSW_Reconnaissance_{method}__"
...
def test_package_maps_runs_and_inventories_omissions(tmp_path: Path):
    ...
    assert sum(row["category"] == "log" for row in omissions) == 5
